...
class NorenApi:
    ...
    def max_payout_amount(self):
        config = NorenApi.__service_config

        # prepare the uri
        url = f"{config['host']}{config['routes']['max_payout_amount']}"
        reportmsg(url)
    ...
    def funds_payout(self, max_amount, amount, ):
        config = NorenApi.__service_config

        # prepare the uri
        url = f"{config['host']}{config['routes']['funds_payout']}"
        reportmsg(url)
        ...
        values["actid"] = self.__accountid
        values["uid"] = self.__username
        values["max_withdr_lmt"] = max_amount
        values["payout"] = amount

        values["email_id"] = 'kushagrar@proton.me'
        values["cliname"] = 'KUSHAGRA RAJPUT'
        values["prfname"] = 'SHOONYA'
        values["acctnum"] = '50100283389612'
        values["bankn"] = 'HDFC%20BANK%20LTD'
        values["ifsc_code"] = 'HDFC0001904'
    ...
    def freeze_account(self ):
        ...
        # prepare the uri
        url = f"{config['host']}{config['routes']['freeze_account']}"
        reportmsg(url)
    ...
    def block_account(self ):
        ...
        # prepare the uri
        url = f"{config['host']}{config['routes']['block_account']}"
        reportmsg(url)
