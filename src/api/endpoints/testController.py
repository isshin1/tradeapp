...
from core.dependencies import get_trade_management, get_shoonya_websocket, get_decision_points_manager
from core.auth import role_checker
router = APIRouter()


@router.get("/api/subscribe/{token}")
async def pnl(
        token: str,
        shoonya_websocket: Annotated[object, Depends(get_shoonya_websocket)]):
        # check_roles: None = Depends(role_checker(["ROLE_ADMIN"]))):
    return shoonya_websocket.subscribe(str(token))
...
async def getGreek(
        price:int,
        name:str,
        decision_points: Annotated[object, Depends(get_decision_points_manager)],
        check_roles: None = Depends(role_checker(["ROLE_ADMIN"]))):
    decision_points.addDecisionPoint(price, name)
...
async def test_order_update(
    ...
    # order_update = {'exchange': 'NSE', 'segment': 'D', 'source': 'T', 'securityId': '83311', 'clientId': '1103209581', 'exchOrderNo': '0', 'orderNo': '35225092667927', 'product': 'I', 'txnType': 'B', 'orderType': 'LMT', 'validity': 'DAY', 'remainingQuantity': 100, 'quantity': 100, 'price': 117.5, 'strategyId': 'NA', 'offMktFlag': '0', 'orderDateTime': '2025-09-26 09:35:38', 'exchOrderTime': '0001-01-01 00:00:00', 'lastUpdatedTime': '2025-09-26 09:35:38', 'remarks': ' ', 'mktType': 'NL', 'reasonDescription': 'RMS:35225092667927:Rate Not Within Ckt Limit 0.05 To 102.25', 'legNo': 1, 'instrument': 'OPTSTK', 'symbol': 'DIVISLAB-Sep202', 'productName': 'INTRADAY', 'status': 'Rejected', 'lotSize': 100, 'strikePrice': 5800, 'expiryDate': '2025-09-30', 'optType': 'PE', 'displayName': 'DIVISLAB 30 SEP 5800 PUT', 'isin': 'NA', 'series': 'XX', 'goodTillDaysDate': '2025-09-26', 'refLtp': 117.95, 'tickSize': 0.05, 'algoId': '0', 'multiplier': 1, 'correlationId': 'TFD'}
    order = {"Data" : order_update }
    trade_management.on_order_update(order)
...
async def getGreek():
    while True:
        ...
        websocketService.send_price_feed("53001", epoch, 24700 + 100 * random.random())
        websocketService.send_price_feed("64694", epoch, 100 + 100 * random.random())
        websocketService.send_price_feed("60484", epoch, 100 + 100 * random.random())
        # websocketService.send_price_feed("48954", epoch, 100 * random.random())
        # websocketService.send_price_feed("53001", epoch, 24800 + 100 * random.random())
        await asyncio.sleep(1)  # Correctly await the sleep coroutine

@router.get("/api/sampleFeedOnce")
async def getGreek():
    # while True:
    epoch = int(datetime.now().timestamp())  # Update epoch in each iteration
    websocketService.send_price_feed("53001", epoch,  24700 + 100 * random.random())
...
async def tradeCheck(
    ...
    # tsym = 'NIFTY ' +  expiry.strftime('%d %b ').upper() + str(strike_price) +  ' ' +optionType
    tsym = trade.tsym
    trade_management.run_feed(time, expiry, tsym, dps)
...
# # async def tradeCheck( background_tasks: BackgroundTasks, trade: TradeRequest ):
# #     price = 165@router.post("/api/tradeCheckOld")
# async def tradeCheck( background_tasks: BackgroundTasks, trade: TradeRequest ):
#     trade.to_datetime()
#     # token = trade.token
#
